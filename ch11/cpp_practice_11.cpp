...
//    istream_iterator<string> inIterator(cin), eof_iterator;
//    vector<string> stringVector3(inIterator, eof_iterator);
...
//    //erase:erase有三个版本：迭代器范围版本，返回后一个迭代器；迭代器版本：删除指定迭代器指向的元素，返回这个元素后一个元素的迭代器；关键字版本：删除关键字对应的元素，返回删除的数量

//    //下标操作：只有map和unorder_map有(at(), [])；如无目标键值则自动插入一个
...
//    }
    ...
    //相邻存储示例
    multimap<string, int> multimap1 = {{"one", 1}, {"two", 2}, {"one", 2}, {"one", 3}};
    ...
    auto num1 = multimap1.count("one");
    auto iterMultimap = multimap1.find("one");
    while(num1--){
    ...
    auto pairMultimap = multimap1.equal_range("one");
